...
const (
	// credentialsJson are the credentials for the GoShot app. This must be regenerated to each app using the library.
	// It should be a member value of the Manager object and passed during its creation (future work).
...
func (m *Manager) createPath(ctx context.Context) (id string, err error) {
	...
	for len(subPath) > 0 {
		...
		if err != nil {
			...
			glog.Errorf("cloud.Manager.createPath: %v", err)
			return "", err
		}
